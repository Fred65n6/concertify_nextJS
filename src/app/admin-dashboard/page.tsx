...
useEffect(() => {
  ...
  const handleDeleteUser = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // try {
    //   const res = await fetch(`/api/data/deleteUser/`, {
    //     method: "DELETE",
    //     body: data,
    //   });
    //   console.log(data);
    ...
    <>
     ...
     <section className="flex gap-4 mt-10">
       ...
       <article className="bg-[#F2EEFB] w-full gap-2 py-8 rounded-lg align-middle justify-start px-8 flex flex-col hover:bg-[#dcd6ea]">
        ...
        <form onSubmit={handleDeleteUser} className="flex flex-col items-center gap-8 pb-12">
           {users?.map((user) => (
            <div key={user._id}>
                  <input
                    readOnly={true}
                    ...
                    value={user._id}
                  />
                  <button type="submit" className="text-[#5311BF]">
